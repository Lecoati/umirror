...
namespace Lecoati.uMirror.Core
{
    ...
    public class Synchronizer
    {
        ...
        {
            MethodInfo method = new BllProject().GetProjectMethod(assemblyRef);
        ...
        public void start()
        {
            ...
            if (!appLock)
            {
                ...
                try
                {
                    ...
                    // Init project
                    Project project = new BllProject().GetProject(_projectid, true);
                    ...
                    if (project != null
                        ...
                        && project.Nodes.Where(r => r.ParentId == null).Count() > 0)
                    {
                        ...
                        if (project.ExtensionMethod != string.Empty && project.ExtensionMethod.Split(';').Count() == 2)
                        {
                        ...
                        Util.UpdateStateAndLogs( _projectName, Lecoati.uMirror.Core.Util.LogType.info, "Loading " + project.XmlFileName, true);
                        ...
                        {
                            int projectIdToTrigger = new BllProject().GetProject(project.TriggerProyect).id;
        ...
        private void sincNavigator(IList<Node> currentNodes, Node parent, string xpathBaseUmbraco, string xpath, int rootParentId, Project project, IEnumerable<XElement> xIElFrom, XElement xElSource)
        {
            ...
            foreach (Node currentNode in currentNodes  /*.Where(r => r.OnlyAdd == false )*/ )
            {
                ...
                IList<Node> NodeChildren = new BllNode().GetNodes(currentNode.id);
        ...
        private bool initProperties(Content doc, XElement xFrom, Node node, Boolean force)
        {
            ...
            foreach (Lecoati.uMirror.Pocos.Property prop in node.Properties)
            {
        ...
        {
            string path = new BllProject().GetProjectFilePath(project);
